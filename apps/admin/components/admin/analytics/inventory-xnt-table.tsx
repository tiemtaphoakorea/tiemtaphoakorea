...
export function InventoryXntTable({
  items,
  totals,
  metadata,
  isLoading,
  ...
  page,
  onPageChange,
}: InventoryXntTableProps) {
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-black tracking-tight text-slate-900">Xuất nhập tồn</h2>
      ...
      <div className="overflow-x-auto rounded-lg ring-1 ring-slate-200">
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading &&
              items.map((row) => {
                const tonCuoi = Number(row.tonCuoi);
                const tonCuoiClass =
                  tonCuoi === 0
                ...
                return (
                  <TableRow key={row.variantId}>
                    ...
                    <TableCell className={`px-4 py-2.5 text-right tabular-nums ${tonCuoiClass}`}>
          ...
          {totals && !isLoading && items.length > 0 && (
            <TableFooter>
              <TableRow>
                <TableCell colSpan={3} className="px-4 py-2.5 font-semibold text-muted-foreground">
