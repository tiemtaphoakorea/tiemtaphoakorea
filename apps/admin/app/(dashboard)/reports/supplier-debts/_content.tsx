...
import { buildExportUrl, reportsClient, type SupplierDebtRow } from "@/services/reports.client";
...
export default function SupplierDebtsContent() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["reports", "supplier-debts", range, search, includeZero, page],
    queryFn: () =>
      reportsClient.getSupplierDebts({
  ...
  function getExportUrl(format: "csv" | "csv-detail" | "xlsx") {
    return buildExportUrl("supplier-debts", { ...range, format, search, includeZero });
  }

  function openDrilldown(row: SupplierDebtRow, focus: "increase" | "decrease") {
    setDrilldown({
      supplierId: row.supplierId,
      supplierName: row.supplierName ?? row.supplierCode ?? "Nhà cung cấp",
      focus,
    });
  }
    ...
    <div className="flex flex-col gap-6 pb-10">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-black tracking-tight">Công nợ nhà cung cấp</h1>
        </div>
        <ReportExportMenu
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <label className="flex cursor-pointer items-center gap-2 text-xs">
          ...
          <span className="font-semibold">Hiện NCC có nợ = 0</span>
        </label>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableEmptyRow cols={5} message="Không có công nợ NCC trong khoảng này." />
            ) : (
              rows.map((r) => (
                <TableRow key={r.supplierId}>
                  <TableCell>
                    <div className="flex flex-col">
                      <span className="text-sm font-semibold">{r.supplierName ?? "—"}</span>
                      <span className="text-xs text-muted-foreground">
                        {[r.supplierCode, r.supplierPhone].filter(Boolean).join(" • ")}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell
                    className={`text-right text-sm tabular-nums ${r.openingDebt < 0 ? "text-emerald-600" : ""}`}
                  >
      ...
      <DebtTransactionsSheet
        target={
          drilldown
            ? { kind: "supplier", id: drilldown.supplierId, name: drilldown.supplierName }
            : null
        }
        startDate={range.startDate}
        endDate={range.endDate}
