import { getSalesByCustomerReport } from "@workspace/database/services/report-sales.server";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const report = await getSalesByCustomerReport({
    ...
    const rows = report.data.map((r) => ({
      ...
      AOV: fmtVnd(r.aov),
      "Lần mua gần nhất": r.lastOrderAt ? fmtDate(r.lastOrderAt) : "",
    }));

    const filename = `doanh-thu-theo-khach-hang-${searchParams.get("startDate")}-${searchParams.get("endDate")}`;
        ...
        [
          {
            ...
            rows,
            columnWidths: [28, 14, 14, 8, 18, 18, 18, 16],
          },
        ],
        `${filename}.xlsx`,
      );
