import { getSalesByTimeReport } from "@workspace/database/services/report-sales.server";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filename = `doanh-thu-theo-thoi-gian-${searchParams.get("startDate")}-${searchParams.get("endDate")}`;

    if (format === "csv") return csvResponse(rows, `${filename}.csv`);

    if (format === "xlsx") {
      return xlsxResponse(
        [{ name: "Doanh thu theo thời gian", rows, columnWidths: [14, 8, 18, 18, 18, 8] }],
        `${filename}.xlsx`,
      );
    }
    ...
    console.error("Failed to export sales by-time report:", error);
    return NextResponse.json(
