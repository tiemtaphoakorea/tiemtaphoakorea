...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filenameBase = `top-kh-doanh-thu-${searchParams.get("startDate")}-${searchParams.get("endDate")}`;
    ...
    if (format === "xlsx") {
      return xlsxResponse(
        [{ name: "Top KH doanh thu", rows, columnWidths: [25, 14, 14, 10, 16, 16, 16] }],
        `${filenameBase}.xlsx`,
      );
    }
    ...
    console.error("Failed to export top-by-revenue report:", error);
    return NextResponse.json(
