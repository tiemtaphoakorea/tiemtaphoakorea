import { getPurchasesBySupplierReport } from "@workspace/database/services/report-purchases.server";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const report = await getPurchasesBySupplierReport({
    ...
    const rows = report.data.map((r) => ({
      "Nhà cung cấp": r.supplierName ?? "(Không rõ)",
    ...
    const filename = `nhap-hang-theo-ncc-${searchParams.get("startDate")}-${searchParams.get("endDate")}`;

    if (format === "xlsx") {
      return xlsxResponse(
        [{ name: "Nhập hàng theo NCC", rows, columnWidths: [28, 14, 10, 10, 18, 18, 18] }],
        `${filename}.xlsx`,
      );
    }
    ...
    console.error("Failed to export purchases by-supplier report:", error);
