...
import { cancelGoodsReceipt } from "@workspace/database/services/goods-receipt.server";
...
export async function POST(request: NextRequest, { params }: IdRouteParams) {
  ...
  if (!CANCEL_ROLES.includes(user.profile.role ?? "")) {
    return NextResponse.json(
      { error: "Chỉ Manager hoặc Owner mới được huỷ phiếu nhập" },
  ...
  try {
    ...
    const updated = await cancelGoodsReceipt(id, user.profile.id);
