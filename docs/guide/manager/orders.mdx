...
<Note>
**Tồn kho**: tạo đơn chỉ tăng `reserved`, chưa trừ `onHand`. Khi sang `stock_out`, hệ thống mới trừ `onHand` và giải phóng `reserved`. Huỷ đơn `pending` chỉ giải phóng `reserved`.
</Note>
