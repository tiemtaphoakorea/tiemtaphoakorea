...
<Note>
**Tồn kho**: Tạo đơn chỉ tăng số giữ chỗ (`reserved`), chưa trừ tồn vật lý (`onHand`). Khi chuyển sang `stock_out`, hệ thống mới trừ `onHand` và giải phóng `reserved`. Huỷ đơn `pending` chỉ giải phóng `reserved`.
</Note>
