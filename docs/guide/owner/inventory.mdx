---
title: "Quản lý kho"
description: "Theo dõi tồn kho từng SKU, phát hiện sản phẩm sắp hết và hết hàng."
---

## Mô tả

Trang **Quản lý kho** cung cấp cái nhìn tổng thể về tồn kho — từng biến thể (SKU) của sản phẩm với 3 chỉ số: **Tồn vật lý**, **Đã giữ chỗ**, **Tồn khả dụng**. Trang cảnh báo các SKU sắp hết hoặc hết hàng để bạn lên kế hoạch nhập hàng kịp thời.

## Cách truy cập

Menu bên trái → **Quản lý kho** (mục **Kho vận**).

## Khái niệm tồn kho

```mermaid
flowchart LR
    P["Tồn vật lý<br/>onHand"] --> A["Tồn khả dụng<br/>available"]
    R["Đã giữ chỗ<br/>reserved"] --> A
    A:::formula

    classDef formula fill:#dbeafe,stroke:#3b82f6,color:#1e40af

    subgraph CT["Công thức"]
        F["available = onHand − reserved"]
    end
```

| Cột | Công thức / nguồn | Ý nghĩa |
|-----|-------------------|---------|
| **Tồn vật lý** (`onHand`) | Số liệu kho thực tế | Số đơn vị thực tế đang trong kho |
| **Đã giữ chỗ** (`reserved`) | Tổng SL trong các đơn chưa giao | Hàng đã bán/đặt nhưng chưa xuất kho |
| **Tồn khả dụng** (`available`) | `onHand − reserved` | Số có thể bán cho đơn mới |

## Dòng chảy tồn kho

```mermaid
flowchart TD
    N["Đơn nhập từ NCC<br/>received"] -->|+SL| OH["Tồn vật lý<br/>(onHand)"]
    O["Tạo đơn hàng KH"] -->|+SL| RV["Đã giữ chỗ<br/>(reserved)"]
    SO["Xuất kho đơn"] -->|−onHand| OH
    SO -->|−reserved| RV
    CN["Huỷ đơn KH"] -->|−reserved| RV
    ADJ["Điều chỉnh thủ công"] -->|±SL| OH

    style OH fill:#dcfce7,stroke:#22c55e
    style RV fill:#fef3c7,stroke:#f59e0b
```

## Cảnh báo nhanh

Banner đỏ ở đầu trang xuất hiện khi có SP sắp hết hoặc hết hàng:

> **N SP sắp hết** · **M SP đã hết hàng**

## Thẻ thống kê

Cụm `MetricStatBar` 4 ô.

| Thẻ | Cách tính |
|-----|-----------|
| **Tổng SKU** | Tổng số biến thể đang quản lý (paginated count, `Đang quản lý`) |
| **Còn hàng** | `MAX(0, totalSkus − lowStock − outOfStock)` (xanh emerald) |
| **Sắp hết** | `COUNT(productVariants)` có `0 < onHand ≤ COALESCE(lowStockThreshold, 30)` (vàng) |
| **Hết hàng** | `COUNT(productVariants)` có `onHand ≤ 0` (đỏ) |

## Tab dữ liệu

Trang có 3 tab — chuyển bằng FilterTabs ở đầu bảng.

### Tab "Tồn kho"

Hiển thị toàn bộ sản phẩm (50 SP đầu, theo phân trang sản phẩm).

| Cột | Nội dung |
|-----|---------|
| **Sản phẩm** | Thumbnail + tên (cắt 40 ký tự) |
| **Danh mục** | Pill màu danh mục (nếu có) |
| **Tồn khả dụng** | Số khả dụng — đỏ khi = 0, vàng khi dưới ngưỡng |
| **Tồn vật lý** | `onHand` — số thực tế |
| **Đã giữ chỗ** | `reserved` — số trong đơn chưa giao |

### Tab "Sắp hết"

Chỉ liệt kê biến thể có `0 < onHand ≤ lowStockThreshold` (mặc định ngưỡng 30). Sắp xếp theo `onHand` tăng dần, lấy top 10 theo từng danh mục.

| Cột | Nội dung |
|-----|---------|
| **Sản phẩm / SKU** | Tên SP + tên biến thể + SKU |
| **Tồn** | `onHand` (chữ vàng đậm) |
| **Tình trạng** | Pill **Sắp hết** (vàng) |

### Tab "Hết hàng"

Liệt kê biến thể có `onHand ≤ 0`. Sắp xếp theo tên sản phẩm, lấy top 10 theo từng danh mục.

| Cột | Nội dung |
|-----|---------|
| **Sản phẩm / SKU** | Tên SP + biến thể + SKU |
| **Tồn** | 0 (chữ đỏ đậm) |
| **Tình trạng** | Pill **Hết hàng** (đỏ) |

## Các thao tác chính

<Steps>
  <Step title="Kiểm tra cảnh báo hàng ngày">
    Mở **Quản lý kho** → đọc banner đỏ ở đầu trang để biết số SKU sắp/hết hàng.
  </Step>
  <Step title="Xem chi tiết SKU sắp hết">
    Nhấn tab **Sắp hết** → bảng hiển thị từng biến thể cụ thể (SKU, tên SP, số tồn còn lại) để lên kế hoạch nhập.
  </Step>
  <Step title="Xem chi tiết SKU đã hết">
    Nhấn tab **Hết hàng** → các SKU cần nhập gấp.
  </Step>
  <Step title="Tạo đơn nhập từ cảnh báo">
    Sang trang **Đơn nhập hàng** → **Tạo đơn nhập** → chọn biến thể tương ứng → nhập số lượng cần nhập.
  </Step>
</Steps>

## Ngưỡng cảnh báo

| Tham số | Mặc định | Cách thay đổi |
|---------|----------|---------------|
| `minLowStockThreshold` | **30** đơn vị | Trang **Sản phẩm** → mở chi tiết sản phẩm → cập nhật ngưỡng phù hợp tốc độ bán |

<Note>
Tạo đơn chỉ tăng **Đã giữ chỗ** (`reserved`), chưa giảm **Tồn vật lý** (`onHand`). `onHand` chỉ giảm khi đơn được **Xuất kho**; huỷ đơn `pending` chỉ giải phóng `reserved`. Phiếu nhập từ NCC cộng vào `onHand` sau khi phiếu nhập được **Hoàn tất**.
</Note>

## Tồn đầu kỳ (cho báo cáo XNT)

Báo cáo Xuất–Nhập–Tồn cần một **điểm xuất phát** — số lượng tồn đầu kỳ. Hệ thống lúc đầu **không tự sinh** giá trị này: Chủ shop / Quản lý cần set thủ công cho từng biến thể khi đối chiếu được với dữ liệu cũ.

<Steps>
  <Step title="Vào sản phẩm">
    Trang **Sản phẩm** → mở sản phẩm → tab **Phiên bản**.
  </Step>
  <Step title="Mở dialog">
    Click icon ⏱ ở cuối row biến thể → dialog **Sửa tồn đầu kỳ** mở ra.
  </Step>
  <Step title="Nhập số tồn đầu thực tế">
    Đối chiếu với phiếu nhập (PON) hoặc kiểm kê cũ → nhập vào ô **Số lượng tồn đầu kỳ mới**.
  </Step>
  <Step title="Lưu">
    Hệ thống tự tính lại chuỗi xuất/nhập của biến thể này và đồng bộ tồn kho hiện tại.
  </Step>
</Steps>

<Warning>
Sửa tồn đầu kỳ làm thay đổi báo cáo XNT lịch sử của biến thể đó. Chỉ thực hiện khi chắc chắn về số mới.
</Warning>

<Warning>
Nút **Điều chỉnh kho** ở góc phải tab hiện chưa có chức năng — đang phát triển. Việc điều chỉnh thủ công (kiểm kê, hao hụt) hiện chưa khả dụng từ giao diện.
</Warning>
