...
export default function AdminProducts() {
  ...
  const limit = Math.max(
    1,
    parseInt(
      searchParams.get("limit") || PAGINATION_DEFAULT.LIMIT.toString(),
      10,
    ),
  );
    ...
    const baseColumns: ColumnDef<ProductListItem>[] = [
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.categoryName ? (
            ...
            <span className="block w-full text-center text-xs text-slate-400 italic">
              -
            </span>
          ),
      },
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const isLowStock =
            !isOutOfStock && row.original.totalStock <= threshold;
